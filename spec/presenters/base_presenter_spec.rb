...
RSpec.describe BasePresenter do
  ...
  describe '.build_with' do
    it "stores ['id', 'title'] in 'build_attributes'" do
      ...
      expect(Presenter.build_attributes).to eq %w(id title)
    end
  end
  ...
  describe '.related_to' do
    it 'stores the correct value' do
      ...
      expect(Presenter.relations).to eq %w(author publisher)
    end
  end
  ...
  describe '.sort_by' do
    it 'stores the correct value' do
      ...
      expect(Presenter.sort_attributes).to eq %w(id title)
    end
  end
